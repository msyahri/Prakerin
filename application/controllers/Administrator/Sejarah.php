...
class Sejarah extends CI_Controller{
  ...
  function index(){
		...
		$this->load->view('backend/v_sejarah',$data);
	...
	function tambah(){
		...
		$this->load->view('backend/main-header');
		$this->load->view('backend/main-sidebar');
		$this->load->view('backend/v_sejarah_input',$data);
		$this->load->view('backend/footer-copyright');
		$this->load->view('backend/control-sidebar');
		$this->load->view('backend/footer-admin');
            ...
            if ($this->upload->do_upload('filefoto'))
            {
                $gbr = $this->upload->data();
								$data = array(
									...
									$this->m_sejarah->input_sejarah($data,'tb_sejarah_desa');
									$this->session->set_flashdata("pesan", "<div class=\"col-md-12\"><div class=\"alert alert-success\" id=\"alert\">Upload gambar berhasil !!</div></div>");
							    redirect('administrator/sejarah'); //jika berhasil maka akan ditampilkan view vupload
							   }
						else
						{
							   $this->session->set_flashdata("pesan", "<div class=\"col-md-12\"><div class=\"alert alert-danger\" id=\"alert\">Gagal upload gambar !!</div></div>");
							    redirect('administrator/sejarah/tambah'); //jika gagal maka akan ditampilkan form upload
							}
					}
	}
	...
	function edit($id){
		$where = array('id' => $id);
		$data['sejarah'] = $this->m_sejarah->edit_data($where,'tb_sejarah_desa')->result();
		$data['title']='Halaman Edit Sejarah';
		$this->load->view('backend/header-admin',$data);
		$this->load->view('backend/main-header');
		$this->load->view('backend/main-sidebar');
		$this->load->view('backend/v_sejarah_edit',$data);
		$this->load->view('backend/footer-copyright');
		$this->load->view('backend/control-sidebar');
		$this->load->view('backend/footer-admin');
	  ...
	  public function edit_aksi(){
		   ...
		   if($_FILES['filefoto']['name'])
		   {
				   if ($this->upload->do_upload('filefoto'))
				   {
					   ...
					   $data = array(
						   ...
						   $this->m_sejarah->get_update($data,$where); //akses model untuk menyimpan ke database
				   ...
				   {  /* jika upload gambar gagal maka akan menjalankan skrip ini */
						   ...
						   redirect('administrator/sejarah/edit'); //jika gagal maka akan ditampilkan form upload
				   }
		   }
		   else
		   { /* jika file foto tidak ada maka query yg dijalankan adalah skrip ini  */
				   ...
				   redirect('administrator/sejarah'); /* jika berhasil maka akan kembali ke home upload */
		   }
	  }
	...
	function hapus($id_sejarah){
		$where = array('id' => $id_sejarah);
		$this->m_sejarah->hapus_data($where,'tb_sejarah_desa');
		redirect('administrator/sejarah/index');
	}
}